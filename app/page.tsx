...
import { loadPrices, type Prices, defaultPrices } from "@/lib/prices"
...
export default function SuenosFelicesLanding() {
  ...
  const [additionalServices, setAdditionalServices] = useState({
    ...
    drawingBoards: 0,
    breakfast: 0,
    pictionaryAir: false, // Nuevo servicio
  })
  ...
  // Temáticas con todas las imágenes reales + nueva temática Lila Bloom
  const tematicas = [
    ...
    { name: "Lila Bloom", image: "/images/lila-bloom.jpg" }, // Nueva temática
  ]

  // Testimonios de clientes actualizados
  const testimonios = [
    {
      texto:
    ...
    {
      texto: "¡Excelente servicio! Súper recomendable todo muy hermoso lleno de detalles. Felicitaciones",
      autor: "Maxi",
  ...
  const calculateTotal = () => {
    ...
    // Precio base de carpitas + carpitas adicionales
    total += prices.tentBase + (selectedTents - 3) * prices.tentAdditional

    // Taller creativo con precios variables según invitados
    if (additionalServices.workshop) {
      if (guestCount <= 6) {
        total += prices.workshop.small
      } else if (guestCount <= 10) {
        total += prices.workshop.medium
      } else {
        total += prices.workshop.large
      }
    }

    if (additionalServices.metegol) total += prices.metegol
    if (additionalServices.pictionaryAir) total += prices.pictionaryAir // Nuevo servicio
    total += additionalServices.kitTrays * prices.kitTrays
    total += additionalServices.drawingBoards * prices.drawingBoards
    total += additionalServices.breakfast * prices.breakfast
  ...
  const getWorkshopPrice = () => {
    if (guestCount <= 6) return prices.workshop.small
    if (guestCount <= 10) return prices.workshop.medium
    return prices.workshop.large
  }
        ...
        <div className="container mx-auto max-w-4xl">
          ...
          <Card className="border-2 border-[#DC9171]/20 shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-[1.02]">
            ...
            <CardContent className="space-y-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-gradient-to-r from-[#DC8C47]/5 to-[#DC9171]/5 p-4 rounded-lg">
                    <div className="text-center space-y-2">
                      <p className="text-[#DC8C47] font-semibold">
                        Precio base (3 carpitas): {formatPrice(prices.tentBase)}
                      </p>
                      {selectedTents > 3 && (
                        <p className="text-[#DC9171] font-medium">
                          + {selectedTents - 3} carpita{selectedTents > 4 ? "s" : ""} adicional
                          {selectedTents > 4 ? "es" : ""}: {formatPrice((selectedTents - 3) * prices.tentAdditional)}
                        </p>
                      )}
                      <p className="text-lg font-bold text-[#DC8C47]">
                        Total carpitas: {formatPrice(prices.tentBase + (selectedTents - 3) * prices.tentAdditional)}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex items-center gap-2 text-[#B2A98C]">
                      ...
                      <span>Cada carpita adicional: {formatPrice(prices.tentAdditional)}</span>
                    </div>
              ...
              {/* Additional Services - Reordenados según solicitud */}
              <div>
                <h3 className="text-lg font-semibold text-[#DC8C47] mb-4">Servicios Adicionales</h3>
                <div className="space-y-4">
                  {/* Pictionary Air - NUEVO SERVICIO PRIMERO */}
                  <div className="flex items-center justify-between p-4 border border-[#DC9171]/20 rounded-lg transition-all duration-300 hover:shadow-md hover:border-[#DC8C47] group">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-[#B2A98C] mt-1">
                        Diversión asegurada con esta versión moderna y digital del clásico Pictionary, ¡Ahora dibujás en
                        el aire!
                      </p>
                      <p className="text-sm font-semibold text-[#DC9171]">{formatPrice(prices.pictionaryAir)}</p>
                    </div>
                    <Button
                      variant={additionalServices.pictionaryAir ? "default" : "outline"}
                      size="sm"
                      onClick={() => updateAdditionalService("pictionaryAir", !additionalServices.pictionaryAir)}
                      className={`transition-all duration-300 hover:scale-105 ${
                        additionalServices.pictionaryAir
                          ? "bg-[#DC8C47] hover:bg-[#DC9171] scale-105"
                          : "border-[#DC8C47] text-[#DC8C47] hover:bg-[#DC8C47] hover:text-white"
                      }`}
                    >
                      {additionalServices.pictionaryAir ? "Añadido" : "Añadir"}
                    </Button>
                  </div>

                  {/* Kit Bandejas y Vasos - SEGUNDO */}
                  <div className="flex items-center justify-between p-4 border border-[#DC9171]/20 rounded-lg transition-all duration-300 hover:shadow-md hover:border-[#DC8C47] group">
                    <div className="flex-1">
                      <span className="font-medium text-[#DC8C47] group-hover:text-[#DC9171] transition-colors duration-300">
                      ...
                      <p className="text-sm font-semibold text-[#DC9171]">{formatPrice(prices.kitTrays)} por carpa</p>
                    </div>
                  ...
                  {/* Tablitas con Actividades - TERCERO */}
                  <div className="flex items-center justify-between p-4 border border-[#DC9171]/20 rounded-lg transition-all duration-300 hover:shadow-md hover:border-[#DC8C47] group">
                    <div className="flex-1">
                      <span className="font-medium text-[#DC8C47] group-hover:text-[#DC9171] transition-colors duration-300">
                      ...
                      <p className="text-sm font-semibold text-[#DC9171]">
                        {formatPrice(prices.drawingBoards)} cada una
                      </p>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Button
                  ...
                  <div className="flex items-center justify-between p-4 border border-[#DC9171]/20 rounded-lg transition-all duration-300 hover:shadow-md hover:border-[#DC8C47] group">
                    <div className="flex-1">
                      ...
                      <p className="text-sm font-semibold text-[#DC9171]">
                        Desde {formatPrice(prices.breakfast)} cada uno
                      </p>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Button
                  ...
                  <div className="flex items-center justify-between p-4 border border-[#DC9171]/20 rounded-lg transition-all duration-300 hover:shadow-md hover:border-[#DC8C47] group">
                    <div className="flex-1">
                      ...
                      <p className="text-sm font-semibold text-[#DC9171]">{formatPrice(prices.metegol)}</p>
                    </div>
                    <Button
                ...
                {calculateTotal() > 0 && (
                  <Button
                    ...
                    onClick={() => {
                      ...
                      if (additionalServices.pictionaryAir) {
                        message += `Pictionary Air: ${formatPrice(prices.pictionaryAir)}\n`
                      }
                      ...
                      if (additionalServices.metegol) {
                        message += `Mini Metegol: ${formatPrice(prices.metegol)}\n`
                      }
